...
df = pd.read_excel(ruta, dtype={"Clave de referencia":str})
codigos = pd.read_excel(r'C:\Users\ypajarino\Projects\2021.02.03 BUSCADOR PDF\codigos enargas.xlsx',index_col = 0,header=None)
...
df["Prod_gas"] = [1 if value > 2000000 or value in [1645, 7914] else 0 for index, value in df["Acreedor"].items()]
...
gas["prov_enargas"] = gas["Acreedor"].replace(codigos).astype(str)
